...
--
-- Host: 127.0.0.1
-- Generation Time: Apr 15, 2021 at 06:27 AM
...
('usertes1', 100000, '2021-04-12', '2021-04-12', 100, 'Belum FIX', 'Belum FIX');
...
('usertes1', 'I Made Eko Satria Wi', 100000, '2021-04-12', '2021-04-12', 'Lunas');
...
--
-- Dumping data for table `rincian_pinjam`
--

INSERT INTO `rincian_pinjam` (`username`, `nama`, `tanggalpinjam`, `tanggalbayar`, `jumlah`, `bunga`, `total`) VALUES
('usertes1', 'usertes1', '2021-04-12', '2021-04-12', 100000, 100, 100000);
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `admin_approve`  AS SELECT `data_pinjam`.`Nama` AS `Nama`, `data_pinjam`.`tanggalpinjam` AS `tanggalpinjam`, `data_pinjam`.`tanggalbayar` AS `tanggalbayar`, `approval`.`status_approve` AS `status_approve` FROM (`data_pinjam` join `approval` on(`data_pinjam`.`username` = `approval`.`username`)) ORDER BY `data_pinjam`.`tanggalpinjam` ASC ;
